...
func _survivor_scene_for(survivor_name: String) -> PackedScene:
	# Map an equipped survivor name to its scene. Extend this map (and add the
	# entry to GameState.CHARACTER_CATALOG) as more survivors are added.
	match survivor_name:
...
func _spawn_flower_items() -> void:
	...
	for i in range(points.size()):
		...
		add_child(item)
	print("GameMap: spawned %d Flower pickups" % points.size())
	for pt in points:
		print("GameMap: Flower at ", pt)
...
func _check_all_survivors_eliminated() -> void:
	...
	if _alive_survivor_bot_count > 0:
		return
	# If the human survivor is already mid-death (fling/fade playing), do NOT
	# cut it short and jump to the lobby. The death sequence itself ends the
	# match after the fade completes — just let it finish.
	if _death_active:
		return
