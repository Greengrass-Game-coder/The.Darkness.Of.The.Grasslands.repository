extends Node
## Tests for the Tetrino coin-reward logic (arcade_room.gd).
...
func _setup() -> void:
	...
	GameState.tetrino_time_penalty_until = 0
	GameState.tetrino_last_seen_time = 0
	GameState.player_money = 0
	GameState.tetrino_gambled = false

func test_first_win_awards_one() -> void:
	_setup()
	GameState.tetrino_coins_earned = 0
	GameState.tetrino_last_coin_time = 0
	_node._hard_mode = false
	var a: int = _node._resolve_win_reward()
	assert(a == 1 and GameState.tetrino_coins_earned == 1 and GameState.player_money == 1,
		"first win should award 1 coin")

func test_same_day_second_win_blocked() -> void:
	_setup()
	GameState.tetrino_coins_earned = 1
	GameState.tetrino_last_coin_time = _node._now_sec()
	_node._hard_mode = false
	var a: int = _node._resolve_win_reward()
	assert(a == 0 and GameState.tetrino_coins_earned == 1,
		"same-day second win should be blocked by daily limit")

func test_next_day_second_win() -> void:
	_setup()
	GameState.tetrino_coins_earned = 1
	GameState.tetrino_last_coin_time = _node._now_sec() - 86400
	_node._hard_mode = false
	var a: int = _node._resolve_win_reward()
	assert(a == 1 and GameState.tetrino_coins_earned == 2,
...
func test_gamble_win_reaches_three() -> void:
	...
	var a: int = _node._resolve_win_reward()
	assert(a == 2 and GameState.tetrino_coins_earned == 3 and GameState.tetrino_gambled,
...
func test_gamble_loss_keeps_one() -> void:
	...
	assert(GameState.tetrino_coins_earned == 1 and GameState.tetrino_gambled,
		"gamble loss should keep 1 coin and lock further earning")

func test_time_tamper_penalty() -> void:
	_setup()
	GameState.tetrino_last_seen_time = _node._now_sec() + 200
	GameState.tetrino_last_coin_time = _node._now_sec() + 100
	_node._detect_time_tamper(_node._now_sec())
	assert(GameState.tetrino_time_penalty_until > _node._now_sec(),
...
func test_admin_unlimited() -> void:
	...
	GameState.tetrino_coins_earned = 1
	GameState.tetrino_last_coin_time = _node._now_sec()  # same day
	_node._hard_mode = false
	var a: int = _node._resolve_win_reward()
	assert(a == 1 and GameState.tetrino_coins_earned == 2,
