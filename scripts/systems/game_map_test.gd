...
func _get_abilities_for(is_killer_player: bool) -> Array[Dictionary]:
	...
	if is_killer_player:
		if _character_name == "Test Killer":
			...
			return [
				{"icon": "res://assets/generated/icon_ability_hit.png", "key": "M1", "name": "M1 HIT", "desc": "Deal 25 damage up to 120px away (2.5s cooldown)", "limit": "25", "cooldown_var": "hit_on_cooldown", "cooldown_timer_var": "hit_cooldown_timer"},
				{"icon": "", "key": "E", "name": "TENTACLE", "desc": "Snag a survivor from range and drag them toward you", "limit": "1000", "cooldown_var": "tentacle_on_cooldown", "cooldown_timer_var": "tentacle_cooldown_timer"},
				{"icon": "", "key": "R", "name": "RAGE", "desc": "Unleash a burst of speed and power", "limit_kind": "rage_traps", "cooldown_var": "rage_on_cooldown", "cooldown_timer_var": "rage_cooldown_timer"},
			]
...
func _update_chase_music(_delta: float) -> void:
	...
	if is_killer:
		...
		# own range, so the full Chase stays on until the survivor runs 2000px away.
	...
	if target_layer != _chase_active_layer:
		...
		if target_layer < 0:
			...
			for i: int in _chase_players.size():
				...
				# Each layer starts from its beginning the moment it becomes the
				# active (audible) layer, instead of resuming mid-track.
				if i == target_layer:
					_chase_players[i].play(0.0)
...
func _on_bot_hp_changed(current_hp: float, _max_hp: float, bot: Node2D) -> void:
	...
	if bot.has_method("play_death_fling"):
		...
		print("GameMapTest: FLING triggered for ", bot.name, " dir=", fling_dir, " mult=", fling_mult)
	...
	match effect_type:
		"teleport":
			if node:
				...
				print("GameMapTest: admin teleported ", target_name, " to (", x, ", ", y, ")")
		"heal":
			if node:
				_apply_admin_heal(node, effect.get("amount", 0.0))
				print("GameMapTest: admin healed ", target_name)
		"damage":
			if node and node.has_method("take_damage"):
				node.take_damage(effect.get("amount", 0.0))
				print("GameMapTest: admin damaged ", target_name)
		"eliminate":
			if node and node.has_method("take_damage"):
				node.take_damage(9999.0)
				print("GameMapTest: admin eliminated ", target_name)
		"set_killer":
			var my_name: String = GameState.logged_in_username.to_lower()
			if target_name.to_lower() == my_name and not GameState.is_killer:
				_switch_role("killer")
			print("GameMapTest: admin made ", target_name, " the killer")
		"spawn_flower":
			if node:
				_spawn_flower_at(node.global_position)
				print("GameMapTest: admin spawned flower for ", target_name)
		"pause":
			get_tree().paused = effect.get("paused", true)
			print("GameMapTest: admin set paused=", effect.get("paused", true))
